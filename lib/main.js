...
$('#start_cap_button').click(function () {
	...
	const astream =video.captureStream();
	streamAudioToWebSocket(astream)
	//video.then(streamAudioToWebSocket) 
      //  .catch(function (error) {
       //     showError('There was an error streaming your audio to Amazon Transcribe. Please //try again.');
     //       toggleStartStop();
     //   });
...
//&& results[0].Alternatives[0].Items.length > 0

    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            let transcript = results[0].Alternatives[0].Transcript;
	    let speaker = results[0].Alternatives[0].Items[0].Speaker;
	console.log("ssssssssssssssss"+speaker);
            ...
            //$('#transcript').val(transcription +"Speaker"+speaker + "=>" + transcript + "\n");
	   // $('#transcript').val(transcription + transcript + "\n");

            // if this transcript segment is final, add it to the overall transcription
            if (!results[0].IsPartial) {
		$('#transcript').text(transcription +"Speaker"+ "=>" + transcript + "\n");
...
$('#stop_cap_button').click(function () {
...
function createPresignedUrl() {
    ...
    return v4.createPresignedURL(
        ...
        crypto.createHash('sha256').update('', 'utf8').digest('hex'), {
            ...
            'expires': 15,
            'region': region,
            'query': "language-code=" + languageCode + "&media-encoding=pcm&sample-rate=" + sampleRate +"&show-speaker-label=true"
        }
    );
}
